...
workflow BACASS {
    ...
    def criteria = multiMapCriteria {
        meta, fastqs, long_fastq, fast5, assembly ->
            ...
            assembly: assembly          != 'NA' ? tuple(meta, assembly) : null
    }
    ...
    ch_input
        ...
        .filter { it != null }
        .map { meta, assembly ->
            tuple(meta, assembly ? file(assembly): null ) // Formato de salida en tupla compatible con outputs de módulos que se skippean
        }
        .set { ch_assembly }
    ...
    if (params.assembly_type != 'long' && params.assembly_type != 'none'){
    ...
    //
    if ( params.assembly_type == 'hybrid' || params.assembly_type == 'long' ) {
        NANOPLOT (
            ch_longreads
        )
